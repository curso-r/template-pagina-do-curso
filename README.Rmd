...
paths <- fs::dir_ls(pasta, glob = "*.html")  |>
...
paths_pdf <- fs::dir_ls(pasta, glob = "*.pdf")  |>
...
dplyr::left_join(paths, paths_pdf, by = "file_path_sem_extencao")  |>
  dplyr::mutate(
    slide = paths,
    link = paste0("https://curso-r.github.io/", params$turma, "/", slide),
...
knitr::kable(
  tibble::tibble(
    script = fs::dir_ls(pasta, glob = "*.R|.*Rmd|.qmd"),
    link = paste0(
...
tibble::tibble(
  nome = fs::dir_ls(pasta, glob = "*.R|.*Rmd|.qmd"),
...
tibble::tibble(
  script = fs::dir_ls(pasta, glob = "*.R|.*Rmd|.qmd|.zip|.rar"),
  link = paste0(
    "https://github.com/curso-r/",
    params$turma,
    "/raw/main/",
